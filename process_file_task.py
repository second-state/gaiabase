...
def task_url(url, process_file_path):
    ...
    try:
        ...
        filename = f"{title}.md"
        subtask_id = create_subtask(first_dir, url, filename, 2)
        output_path = Path(process_file_path) / filename
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(plain_text)
        ...
        update_subtask(subtask_id, 1,-1)
        err_output_path = Path(first_dir) / "err_files" / f"{Path(sanitized_url).stem}_err.txt"
        with open(err_output_path, 'w', encoding='utf-8') as f:
            f.write(str(e))
...
def task_md(src_path, dest_dir, subtask_id):
    try:
        dest_dir = Path(dest_dir)
        dest_dir.mkdir(parents=True, exist_ok=True)

        dest_path = dest_dir / f"{Path(src_path).name}"
